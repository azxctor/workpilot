...
时间前缀便于人类识别会话的创建时间；随机后缀避免同秒启动两个实例时
文件名冲突。

**排序不依赖文件名。** 实现中发现：文件名的字典序只在秒级不同时等于
时间序 —— 同一秒创建的两个会话，排序完全由随机后缀决定，先创建的会
被当成最新的。因此 `--continue` 按文件 **mtime** 排序。这同时让语义
更正确：想要的是「最近活动过的」会话，而非「最近创建的」——昨天建、
今天又聊过的会话才是用户想接着聊的那个。

### 文件延迟创建

`SessionStore.create()` **不立即落盘**，文件在第一条消息到来时才创建
（连带写入 meta 行）。

实现时发现：若启动即创建文件，用户启动后立刻退出就会留下一个只有
meta 的空会话。危害不止是垃圾文件 —— 该文件 mtime 最新，**下次
`--continue` 会匹配到这个空会话**，真正想恢复的会话被跳过，上下文丢失。

延迟创建比「退出时清理」更彻底：进程被 kill 时也不会留下残骸。
...
| 会话文件仅有 meta 行（旧版本遗留） | 视为空会话，正常加载不报错 |
| 启动后未发言即退出 | 不产生任何文件（延迟创建） |
